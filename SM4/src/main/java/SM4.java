...
public class SM4 {
    ...
    private static int[] keyGenerate(byte[] key_t) {
        ...
        final int[] CK = {0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269, 0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9, 0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249, 0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9, 0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229, 0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299, 0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209, 0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279};
    ...
    public byte[] SM4_CFB_Encrypt(byte[] plaintext) {
        int len_padding = 16 - plaintext.length % 16;
        byte[] ciphertext = new byte[plaintext.length + len_padding];
        ...
        for (index = 0; index < (plaintext.length >>> 4); index++) {
            temp = subByteArray(plaintext, index * 16, 16);
            temp1 = sm4Main(IV, this.key, 0);
            for (int j = 0; j < 16; j++) {
                ciphertext[index * 16 + j] = (byte) (temp1[j] ^ temp[j]);
                IV[j] = ciphertext[index * 16 + j];
            }
//            for (int j = 0; j < 16; j++) {
//                ciphertext[index * 16 + j] = temp1[j];
//                IV[j] = temp1[j];
//            }
        ...
        temp = subByteArray(plaintext, index * 16, 16 - len_padding);
        temp1 = PKCS7(temp);
        temp2 = sm4Main(IV, this.key, 0);
        for (int j = 0; j < 16; j++) {
            ciphertext[index * 16 + j] = (byte) (temp1[j] ^ temp2[j]);
        }
        return ciphertext;
    }

    public byte[] SM4_CFB_Decrypt(byte[] ciphertext) {
        byte[] plaintext = new byte[ciphertext.length];
        byte[] temp, temp1;
        byte[] IV = new byte[16];
        System.arraycopy(this.IV, 0, IV, 0, 16);
        for (int i = 0; i < (ciphertext.length >>> 4); i++) {
            temp = subByteArray(ciphertext, i * 16, 16);
            temp1 = sm4Main(IV, this.key, 0);                         //加密模式
            for (int j = 0; j < 16; j++) {
                plaintext[i * 16 + j] = (byte) (temp1[j] ^ temp[j]);
                IV[j] = temp[j];
            }
        }
        int padding_len = plaintext[plaintext.length - 1];
    ...
    public byte[] SM4_OFB_Encrypt(byte[] plaintext) {
        int len_padding = 16 - plaintext.length % 16;
        byte[] ciphertext = new byte[plaintext.length + len_padding];
...
//            }
    ...
    public byte[] SM4_OFB_Decrypt(byte[] ciphertext) {
        byte[] plaintext = new byte[ciphertext.length];
        byte[] temp, temp1;
        byte[] IV = new byte[16];
        System.arraycopy(this.IV, 0, IV, 0, 16);
