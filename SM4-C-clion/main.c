...
void SM4Main(unsigned char *inputText, unsigned char *outputText, int mode) {
    ...
    int index = 0;
    for (i = 0; i < 4; i++) {
        X_circle[i] = charToLong(inputText[i * 4], inputText[i * 4 + 1], inputText[i * 4 + 2], inputText[i * 4 + 3]);
    }
    for (i = 0; i < 32; i++) {
        if (mode == 0) {
            index = i;
...
//                             0xfe, 0xdc, 0xba, 0x98,
//                             0x76, 0x54, 0x32, 0x10};
    unsigned char key[16] = "8765432187654321";
    unsigned char plaintext[16] = "1234567812345678";
